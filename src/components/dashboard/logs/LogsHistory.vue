...
<script setup>import dayjs from '@/lib/dayjs';
...
const SharedCodemirror = defineAsyncComponent(() => import('@/components/dashboard/shared/SharedCodemirror.vue'));
...
const translatedLog = computed(() => props.currentLog.history.map(translateLog));
const filteredLog = computed(() => {
    const query = state.search.toLocaleLowerCase();
    return translatedLog.value.filter((log) => log.name.toLocaleLowerCase().includes(query) ||
        log.description?.toLocaleLowerCase().includes(query));
});
const history = computed(() => filteredLog.value.slice((pagination.currentPage - 1) * pagination.perPage, pagination.currentPage * pagination.perPage));
const errorBlock = computed(() => {
    if (props.currentLog.status !== 'error')
        return null;
    let block = props.currentLog.history.at(-1);
    if (!block || block.type !== 'error' || block.id < 25)
        return null;
...
const logCtxData = computed(() => {
    ...
    if (logData.ctxData)
        logData = logData.ctxData;
    if (!state.itemId || !logData[state.itemId])
        return '';
    const data = logData[state.itemId];
    if (data?.referenceData)
        getDataSnapshot(data.referenceData);
    const itemLogData = state.activeTab === 'all' ? data : objectPath.get(data, state.activeTab);
    return JSON.stringify(itemLogData, null, 2);
});
const getDataSnapshot = (refData) => {
    if (!props.ctxData?.dataSnapshot)
        return;
...
const exportLogs = (type) => {
    ...
    const getItemData = {
        'plain-text': ([timestamp, status, name, description, message, ctxData,]) => {
            data += `${timestamp} - ${status} - ${name} - ${description} - ${message} - ${JSON.stringify(ctxData)} \n`;
        },
    ...
    translatedLog.value.forEach((item, index) => {
        ...
        if (itemData)
            getDataSnapshot(itemData.referenceData);
        getItemData[type]([
    ...
    const filename = `[${dayjs().format('DD-MM-YYYY, HH:mm:ss')}] ${props.currentLog.name} - logs`;
...
const translateLog = (log) => {
    ...
    if (['finish', 'stop'].includes(log.type)) {
        ...
        copyLog.name = getTranslatation(`workflow.blocks.${log.name}.name`, blocks[log.name].name);
    }
    if (copyLog.message && messageHasReferences.includes(copyLog.message)) {
        copyLog.messageId = `${copyLog.message}`;
    }
    copyLog.message = getTranslatation({ path: `log.messages.${log.message}`, params: log }, log.message);
    return copyLog;
};
const setActiveLog = (item) => {
    state.itemId = item.id;
    activeLog.value = item;
};
const getBlockPath = (blockId) => {
    const { workflowId } = props.currentLog;
