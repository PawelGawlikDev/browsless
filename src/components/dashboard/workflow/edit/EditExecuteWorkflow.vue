<template>
  <div class="mb-12">
    ...
    <template v-if="!data.insertAllVars">
      ...
      <span class="ml-1 block text-sm leading-tight text-gray-600 dark:text-gray-200">
        {{ t('workflow.blocks.execute-workflow.useCommas') }}
      </span>
    </template>
    <ui-modal v-model="state.showGlobalData" title="Global data" content-class="max-w-xl">
...
const filterWorkflows = (workflows) =>
  workflows
    .filter(({ id, drawflow }) => {
      const flow = typeof drawflow === 'string' ? drawflow : JSON.stringify(drawflow);
