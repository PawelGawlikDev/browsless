<template>
  <div class="mb-2 mt-4">
    ...
    <template v-if="!data.everyNewTab">
      ...
      >
        <option v-for="item in ['website', 'background']" :key="item" :value="item">
        ...
        <ui-tab-panel value="code" class="h-full">
          ...
          <template v-if="!data.everyNewTab">
            <p class="mt-1 flex justify-between text-sm">
              <span>{{ t('workflow.blocks.javascript-code.availabeFuncs') }}</span>
              <span>
                <span
...
const availableFuncs = [
  { name: 'browslessNextBlock(data, insert?)', id: 'browslessnextblock-data' },
  { name: 'browslessRefData(keyword, path?)', id: 'browslessrefdata-keyword-path' },
  {
    name: 'browslessSetVariable(name, value)',
    id: 'browslesssetvariable-name-value',
  },
  {
    name: 'browslessFetch(type, resource)',
    id: 'browslesssetvariable-type-resource',
  },
  { name: 'browslessResetTimeout()', id: 'browslessresettimeout' },
];
const autocompleteList = Object.values(browslessFuncsSnippets).slice(0, 4);
