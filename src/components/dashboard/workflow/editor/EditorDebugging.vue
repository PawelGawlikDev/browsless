  ...
  >
    <div class="mr-4 w-52">
      <div class="flex items-center gap-2">
        ...
        >
          <v-remixicon
            :name="workflowState.status === 'breakpoint' ? 'riPlayLine' : 'riPauseLine'"
            class="mr-2 -ml-1"
          />
          <span>
            {{
              t(`common.${workflowState.status === 'breakpoint' ? 'resume' : 'pause'}`)
            }}
          </span>
        </ui-button>
        <ui-button
      ...
      <ui-list v-if="workflowState.state" class="mt-4 overflow-auto h-[105px] scroll">
        <ui-list-item
        ...
        >
          <div class="text-overflow text-sm w-full">
            ...
            <p class="leading-tight text-overflow text-gray-600 dark:text-gray-200">
    ...
    <div class="w-64">
      ...
      <ui-tab-panels v-model="activeTab">
        ...
        >
          <ui-list class="mt-2">
            <ui-list-item
              v-for="item in (workflowState?.state?.logs ?? []).slice(-100).reverse()"
              :key="item.id"
              small
              class="!block"
            >
...
const toggleExecution = () => {
  ...
  if (workflowState.value.status === 'running') {
    ...
    sendMessage('workflow:resume', { id: workflowState.value.id }, 'background');
  }
};
const stopWorkflow = () => {
  if (!workflowState.value) return;
