...
type RemixiconComponent = DefineComponent<{
  name?: string;
  title?: string;
  viewBox?: string;
  size?: string | number;
...
const component = {
  ...
  setup(props: {
    ...
    const injectIcons = inject<Record<string, string>>('remixicons') ?? {};
    const icon = computed(() => {
      if (props.path) return props.path;

      const iconStr = props.name ? injectIcons[props.name] : undefined;
...
export default {
  install(app: App) {
    ...
    app.component('VRemixicon', component as unknown as RemixiconComponent);
  },
};
