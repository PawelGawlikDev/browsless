import findSelector from '@/lib/findSelector';
import type { ListSelectorOptions } from './types';
export const getAllSiblings = (el: Element, selector?: string) => {
  const siblings = [el];
  const validateElement = (element: Element) => {
    const isValidSelector = selector ? element.querySelector(selector) : true;
    const isSameTag = el.tagName === element.tagName;
    return isValidSelector && isSameTag;
  };
  let nextSibling = el;
  let prevSibling = el;
  let elementIndex = 1;
  while ((prevSibling = prevSibling?.previousElementSibling)) {
    if (validateElement(prevSibling)) {
      elementIndex += 1;
      siblings.unshift(prevSibling);
    }
  }
  while ((nextSibling = nextSibling?.nextElementSibling)) {
    if (validateElement(nextSibling)) {
      siblings.push(nextSibling);
    }
  }
  return {
    elements: siblings,
    index: elementIndex,
  };
};
export const getCssPath = (el, root = document.body) => {
  if (!el) return null;
  const path: string[] = [];
  while (el.nodeType === Node.ELEMENT_NODE && !el.isSameNode(root)) {
    let selector = el.nodeName.toLowerCase();
    if (el.id) {
      selector += `#${el.id}`;
      path.unshift(selector);
    } else {
      let nth = 1;
      let sib: Element | null = el;
      while ((sib = sib.previousElementSibling)) {
        if (sib.nodeName.toLowerCase() === selector) nth += 1;
      }
      if (nth !== 1) selector += `:nth-of-type(${nth})`;
      path.unshift(selector);
    }
    el = el.parentElement;
  }
  return path.join(' > ');
};
export const getElementList = (
  el: Element | null,
  maxDepth = 50,
  paths: string[] = []
): Element[] | null => {
  if (maxDepth === 0 || !el || el.tagName === 'BODY') return null;
  let selector = el.tagName.toLowerCase();
  const { elements, index } = getAllSiblings(el, paths.join(' > '));
  let siblings = elements;
  if (index !== 1) selector += `:nth-of-type(${index})`;
  paths.unshift(selector);
  if (siblings.length === 1) {
    siblings = getElementList(el.parentElement, maxDepth - 1, paths);
  }
  return siblings;
};
export default function (
  target: Element | null,
  { frameElement, onlyInList, selectorSettings }: ListSelectorOptions = {}
) {
  if (!target) return [];
  const browslessListEl = target.closest('[browsless-el-list]');
  let documentCtx: Document | Element = document;
  if (frameElement) {
    documentCtx = frameElement.contentDocument ?? document;
  }
  if (browslessListEl) {
    if (target.hasAttribute('browsless-el-list')) return [];
    const childSelector = findSelector(target, {
      root: browslessListEl,
      ...(selectorSettings || {}),
      idName: () => false,
    });
    const elements = documentCtx.querySelectorAll(`[browsless-el-list] ${childSelector}`);
    return Array.from(elements);
  }
  if (onlyInList) return [];
  return getElementList(target) || [target];
}
