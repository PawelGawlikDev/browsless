<template>
  ...
  <div v-if="packageState.settings.asBlock" class="mt-6 flex space-x-6 pb-8">
    <div class="flex-1">
      ...
      <div class="mt-4">
        <div v-if="packageState.inputs.length > 0" class="grid grid-cols-12 gap-x-4">
        ...
        >
          <template #item="{ element, index }">
            <div class="group relative mb-2 grid grid-cols-12 items-center gap-x-4">
              <span
    ...
    <div class="flex-1">
      ...
      <div class="mt-4">
        <div v-if="packageState.outputs.length > 0" class="grid grid-cols-12 gap-x-4">
...
const getBlockIOName = (type, data) => {
  ...
  if (!node) {
    ...
    const handleName = index === -1 ? 'Not found' : `${type.slice(0, -1)} ${index + 1}`;
