...
import type { BrowserApiEventController, BrowserApiMapItem } from '@/types/runtime';
...
const onBrowserAPIEvent = (name, ...args) => {
  MessageListener.sendMessage(BROWSER_API_EVENTS.ON_EVENT, { name, args }, 'offscreen');
};
/**
...
class BrowserAPIEventHandler {
  ...
  #eventsHandler: Record<string, BrowserApiEventController>;
  /** @type {Set<string>} */
  #isEventAdded;
  /** @type {Record<string, ((...args: unknown[]) => void)>} */
  #browserEvents: Record<string, (...args: unknown[]) => void>;
  constructor() {
    this.#events = {};
    this.#browserEvents = {};
    this.#eventsHandler = {};
   ...
   * @param {*} browserAPI
   */
  createEventListener(name: string): BrowserApiEventController {
     ...
     */
    const addListener = (callback: (...args: unknown[]) => void) => {
     ...
     * @param {eventListenerCallback} callback
     */
    const removeListener = (callback: (...args: unknown[]) => void) => {
     ...
     * @param {eventListenerCallback} callback
     */
    const hasListener = (callback: (...args: unknown[]) => void) => {
   ...
   */
  onBrowserEventListener(event: { name: string; args: unknown[] }) {
   ...
   */
  onToggleBrowserEventListener({ name, type }: { name: string; type: 'add' | 'remove' }) {
    const isAddListener = type === 'add';
    if (isAddListener && this.#browserEvents[name]) return;
    if (!isAddListener && !this.#browserEvents[name]) return;
    const browserEventAPI = (browserAPIMap
      .find((item: BrowserApiMapItem) => item.path === name && item.isEvent)
      ?.api() ?? null) as BrowserApiEventController | null;
    if (!browserEventAPI) return;
