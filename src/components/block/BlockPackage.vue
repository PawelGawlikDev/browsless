  ...
  >
    <div class="flex items-center">
      ...
      >
        ...
        />
        <span class="text-overflow">{{ data.name || 'Unnamed block' }}</span>
      </div>
      <div class="grow" />
      <v-remixicon
...
const installPackage = () => {
  packageStore.insert({ ...props.data, isExternal: false }, false).then(() => {
    state.isInstalled = true;
  });
};
const removeConnections = (type, old, newEdges) => {
  const removedEdges = [];
  old.forEach((edge) => {
