<template>
  <edit-interaction-base v-bind="{ data, hide: hideBase }" @change="updateData">
    ...
    <div class="mt-3 space-y-2">
      ...
      <ui-checkbox :model-value="data.smooth" @change="updateData({ smooth: $event })">
        {{ t('workflow.blocks.element-scroll.smooth') }}
      </ui-checkbox>
      <template v-if="!data.scrollIntoView">
        <ui-checkbox :model-value="data.incX" @change="updateData({ incX: $event })">
          {{ t('workflow.blocks.element-scroll.incScrollX') }}
        </ui-checkbox>
        <ui-checkbox :model-value="data.incY" @change="updateData({ incY: $event })">
