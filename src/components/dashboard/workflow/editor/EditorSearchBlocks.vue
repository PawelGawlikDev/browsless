<template>
  <div class="ml-2 inline-flex items-center rounded-lg bg-white dark:bg-gray-800">
    <button
      v-tooltip="
    ...
    >
      ...
      <template #item="{ item }">
        <div class="flex-1 overflow-hidden">
          ...
          <p class="text-overflow text-sm leading-none text-gray-600 dark:text-gray-300">
            {{ item.description }}
          </p>
        </div>
        <span
...
const searchNodes = ({ item, text }) => {
  const isMatch = (str) => str.toLocaleLowerCase().includes(text.toLocaleLowerCase());
...
const onSelectItem = ({ item }) => {
  ...
  clearHighlightedNodes();
  document.querySelector(`[data-id="${item.id}"]`)?.classList.add('search-select-node');
