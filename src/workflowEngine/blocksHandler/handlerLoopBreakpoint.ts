...
type LoopBreakpointBlockData = {
  loopId: string;
  clearLoop?: boolean;
};

type LoopElementsResult = Array<unknown> & {
  continue?: boolean;
  length: number;
};

async function loopBreakpoint(
  this: WorkflowHandlerContext,
  block: WorkflowHandlerBlock<LoopBreakpointBlockData>,
  { prevBlockData }: { prevBlockData: unknown }
) {
