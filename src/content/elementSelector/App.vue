<template>
  ...
  >
    ...
    >
      ...
      >
        <v-remixicon name="riDragMoveLine" @mousedown="state.isDragging = true" />
      </div>
      <div class="flex items-center px-4 pt-4">
        <p class="text-lg font-semibold">Browsless</p>
        <div class="grow"></div>
        <button
      ...
      <div class="p-4">
        ...
        <selector-elements-detail
          v-if="!state.showSettings && !state.hide && state.selectedElements.length > 0"
        ...
        />
        <div
          v-if="state.showSettings && state.selectorType === 'css' && !state.hide"
          class="mt-4"
        >
            ...
            <li>
              ...
              <template v-if="selectorSettings.attr">
                <label
                  ...
                  for="browsless-attribute-names"
                >
                  Attribute names
                </label>
                <ui-textarea
                  id="browsless-attribute-names"
                  v-model="selectorSettings.attrNames"
                  label="Attribute name"
                  placeholder="data-testid, aria-label, type"
                />
                <span class="text-sm"> Use commas to separate the attribute </span>
              </template>
            </li>
          </ul>
        </div>
        <p class="mt-1 text-sm text-gray-600">
          Click or press
          <kbd class="bg-box-transparent rounded-md p-1">Space</kbd> to select an element
        </p>
      </div>
    </div>
  </div>
...
const toggleHighlightElement = ({ index, highlight }) => {
  state.selectedElements[index].highlight = highlight;
};
const onElementsSelected = ({ selector, elements, path, selectElements }) => {
  if (path) {
...
const onMousemove = ({ clientX, clientY }) => {
  ...
  else if (cardRect.height + clientY > height) clientY = height - cardRect.height;
  if (clientX < 10) clientX = 10;
  else if (cardRect.width + clientX > width) clientX = width - cardRect.width;
  cardRect.x = clientX;
  cardRect.y = clientY;
};
const selectElementPath = (type) => {
  let pathIndex =
...
const onMessage = ({ data }) => {
  if (data.type !== 'browsless:selected-elements') return;
...
const destroy = () => {
  ...
  prevSelectedList.forEach((element) => {
    element.removeAttribute('browsless-el-list');
  });
  document.documentElement.style.fontSize = originalFontSize;
};
const clearConnectedPort = () => {
...
watch(
  () => state.isDragging,
  (value) => {
    document.body.toggleAttribute('browsless-isDragging', value);
  }
);
watch(
  selectorSettings,
...
onMounted(() => {
  extensionStorage.local.get('selectorSettings').then((storage) => {
