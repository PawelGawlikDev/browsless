                ...
                <ul class="space-y-1">
                  ...
                  >
                    ...
                    <v-remixicon
                      name="riDeleteBin7Line"
                      class="invisible cursor-pointer group-hover:visible"
                      @click="state.sources[index].destinations.splice(destIndex, 1)"
                    />
                  </li>
                </ul>
                <ui-button icon class="mt-2 text-sm" @click="addDestination(index)">
...
const isNameDuplicate = ({ items, currItem, newName, event }) => {
...
const updateSource = ({ index, source, event }) => {
...
const updateDestination = ({ index, destIndex, destination, event }) => {
  const newName = event.target.value.trim();
...
const addDestination = (sourceIndex) => {
  const id = nanoid(4);
