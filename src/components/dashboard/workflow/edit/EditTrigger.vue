<template>
  <div class="trigger">
    ...
    <ui-modal v-model="state.showParamModal" title="Parameters" content-class="max-w-4xl">
    ...
    >
      <shared-workflow-triggers :triggers="state.triggers" @update="updateWorkflow" />
    </ui-modal>
  </div>
</template>
<script setup>
...
const updateWorkflow = (triggers) => {
  state.triggers = triggers;
  updateData({ triggers });
};
onMounted(() => {
  if (props.data.triggers) return;
  state.triggers = [{ type: props.data.type, data: { ...props.data }, id: nanoid(5) }];
});
</script>
<style>
.trigger-item > button {
