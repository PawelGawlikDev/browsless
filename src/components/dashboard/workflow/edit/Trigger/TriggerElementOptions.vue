  ...
  <ul class="space-y-2">
    <li v-for="option in types" :key="option" class="group">
      <ui-checkbox
        ...
        @change="$emit('update:modelValue', { ...modelValue, [option]: $event })"
      >
        {{ t(`workflow.blocks.trigger.element-change.${option}.title`) }}
        <v-remixicon
          :title="t(`workflow.blocks.trigger.element-change.${option}.description`)"
      ...
      <template v-if="option === 'attributes' && modelValue.attributes">
        <ui-input
          ...
          :label="t('workflow.blocks.trigger.element-change.subtree.description')"
          class="block w-full"
        ...
        >
          <template #label>
            ...
            <v-remixicon
              :title="
                t('workflow.blocks.trigger.element-change.attributeFilter.description')
              "
              class="info-icon"
              size="18"
              name="riInformationLine"
