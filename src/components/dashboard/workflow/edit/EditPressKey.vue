<template>
  <div>
    ...
    <div v-if="!data.action || data.action === 'press-key'" class="flex items-end">
      <ui-autocomplete
        :items="keysList"
      ...
      <ui-button
        v-tooltip="
          isRecordingKey ? t('common.cancel') : t('workflow.blocks.press-key.detect')
        "
        icon
        class="ml-2"
...
const attachKeyEvents = () => {
  window.addEventListener('keyup', onKeyup);
  window.addEventListener('keydown', onKeydown);
};
const detachKeyEvents = () => {
