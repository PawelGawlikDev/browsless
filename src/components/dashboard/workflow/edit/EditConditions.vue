<template>
  <div>
    ...
    <div class="my-4 flex items-center space-x-2">
      ...
      >
        <v-remixicon :name="state.showSettings ? 'riCloseLine' : 'riSettings3Line'" />
      </ui-button>
    </div>
    ...
    <ui-modal v-model="state.showModal" custom-content>
      <ui-card padding="p-0" class="w-full max-w-3xl">
        ...
        <div class="scroll overflow-auto p-4" style="height: calc(100vh - 8rem)">
          <input
...
import SharedConditionBuilder from '@/components/dashboard/shared/SharedConditionBuilder/index.vue';
...
const deleteCondition = (index, id) => {
  conditions.value.splice(index, 1);
  props.editor.removeEdges((edges) => {
    return edges.filter((edge) => edge.sourceHandle === `${props.blockId}-output-${id}`);
  });
};
