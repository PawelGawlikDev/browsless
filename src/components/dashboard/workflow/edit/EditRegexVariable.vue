  ...
  <div>
    ...
    <div class="mt-3 flex items-end">
      <div class="mr-2 flex-1">
        ...
        <div class="bg-input flex items-center rounded-lg px-4 transition-colors">
          <span>/</span>
          <input
            id="var-expression"
            ...
            @input="updateData({ expression: ($event.target as HTMLInputElement).value })"
          />
...
const props = defineProps({
  data: {
    type: Object as () => RegexVariableData,
    default: () => ({ flag: [] }),
  },
});
const emit = defineEmits<{
  'update:data': [value: RegexVariableData];
}>();
const methods: RegexMethod[] = [
...
const { t } = useI18n();
const updateData = (value: Partial<RegexVariableData>) => {
  emit('update:data', { ...props.data, ...value });
};
const updateFlag = (include: boolean, flag: string) => {
