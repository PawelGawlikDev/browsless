    ...
    >
      <template #header-append>
        <div>
          ...
          <ui-button class="ml-4" variant="accent" @click="updateWorkflowTrigger">
            {{ t('common.save') }}
          </ui-button>
        </div>
      </template>
...
import { registerWorkflowTrigger, workflowTriggersMap } from '@/utils/workflowTrigger';
import SharedWorkflowTriggers from '@/components/dashboard/shared/SharedWorkflowTriggers.vue';
const { t } = useI18n();
...
const getTriggersData = async (triggerData, { id, name }) => {
  try {
...
const getWorkflowTrigger = async (workflow, { location, path }) => {
...
const iterateWorkflows = ({ workflows, path, location }) => {
...
const clearAddWorkflowSchedule = () => {
...
const updateWorkflowTrigger = async () => {
  try {
    const { triggers: workflowTriggers, id, name } = scheduleState.selectedWorkflow;
    ...
    state.triggers = state.triggers.filter((trigger) => {
      const isNotMatch = scheduleState.selectedWorkflow.id !== trigger.workflowId;
      if (!isNotMatch) {
