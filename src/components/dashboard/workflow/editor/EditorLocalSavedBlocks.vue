<template>
  <div class="absolute bottom-0 z-50 w-full p-4">
    <ui-card class="h-full w-full" padding="p-0">
      ...
      >
        <p v-if="packageStore.packages.length === 0" class="w-full py-8 text-center">
          {{ t('message.noData') }}
        </p>
        <div
          v-for="item in items"
          ...
          @dragstart="$event.dataTransfer.setData('savedBlocks', JSON.stringify(item))"
        >
          <div class="flex flex-1 items-start p-4">
            ...
            >
              <img
                ...
                class="rounded-lg"
              />
              <v-remixicon v-else :name="item.icon || 'mdiPackageVariantClosed'" />
            </div>
          ...
          >
            ...
            <ui-popover style="height: 18px">
              <template #trigger>
                <v-remixicon size="18" class="cursor-pointer" name="riMore2Line" />
              </template>
              <ui-list>
                <ui-list-item
                  v-close-popover
                ...
                >
                  Update library item
                  <v-remixicon
                    v-tooltip="'Update the current library item inside the workflow.'"
                    class="ml-2 -mr-1"
...
const removeConnections = ({ id, type, oldEdges, newEdges }) => {
  const removedEdges = [];
