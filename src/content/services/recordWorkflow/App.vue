<template>
  ...
  >
    ...
    >
      ...
      <p class="ml-2 font-semibold">Browsless</p>
      <div class="grow"></div>
        ...
        <template v-else>
          ...
          <template v-else>
            <div class="flex w-full items-center space-x-2">
              ...
              />
              <template
                v-if="!selectState.list && !selectState.childSelector.includes('|>')"
              >
            ...
            <template
              v-if="['get-text', 'attribute-value'].includes(addBlockState.activeBlock)"
            >
              <select
              ...
              <label for="variable-name" class="ml-2 mt-2 text-sm text-gray-600">
                Assign to variable
              </label>
              <input
                id="variable-name"
                ...
                class="bg-input w-full rounded-lg px-4 py-2"
              />
              <label for="select-column" class="ml-2 mt-2 text-sm text-gray-600">
                Insert to table
              </label>
              <select
                id="select-column"
  ...
  <shared-element-selector
    ...
    :pause="
      selectState.selectedElements.length > 0 && selectState.list && !selectState.listId
    "
...
const onElementsSelected = ({ selector, elements, path }) => {
  if (path) {
...
const clearSelectState = () => {
  ...
  const selectedList = document.querySelectorAll('[browsless-el-list]');
  ...
  document.body.removeAttribute('browsless-selecting');
};
const saveElementListId = () => {
...
const toggleDragging = (value, event) => {
  if (value) {
...
const startSelecting = (list = false) => {
  selectState.list = list;
  selectState.isSelecting = true;
  selectState.status = 'selecting';
  document.body.setAttribute('browsless-selecting', '');
