<template>
  <div>
    ...
    <template v-if="permission.has.cookies">
      ...
      <template v-else>
        ...
        <ui-input
          ...
          :label="`Name ${data.type === 'get' && !data.getAll ? '' : '(optional)'}`"
          class="mt-2 w-full"
    ...
    <template v-else>
      <p class="mt-4">This block requires "Cookies" permission to work properly</p>
...
const isGetOrSet = computed(
  () => (props.data.type === 'get' && props.data.getAll) || props.data.type === 'set'
);
const cookieOptionsAutocomplete = (context) => {
  const word = context.matchBefore(/\w*/);
  const nodeBefore = syntaxTree(context.state).resolveInner(context.pos, -1);
  if (nodeBefore.name !== 'PropertyName' || (word.from === word.to && !context.explicit))
    return null;
  let options = [];
