<template>
  <div class="container py-8 pb-4">
      ...
      <div class="mr-8 hidden w-60 lg:block">
        <div class="flex items-center">
          ...
          <ui-popover>
            ...
            <ui-list>
              <ui-list-item v-close-popover class="cursor-pointer" @click="importPackage">
                {{ t('packages.import') }}
              </ui-list-item>
            </ui-list>
          </ui-popover>
      ...
      <div class="flex-1">
        <div class="flex flex-wrap items-center">
          ...
          <div class="workflow-sort mt-4 flex items-center lg:mt-0">
            <ui-button
              ...
              @click="sortState.order = sortState.order === 'asc' ? 'desc' : 'asc'"
            >
              <v-remixicon
        ...
        >
          ...
          >
            ...
            <router-link :to="`/packages/${pkg.id}`" class="mt-4 flex-1 cursor-pointer">
              <p class="text-overflow font-semibold">
                {{ pkg.name }}
              </p>
              <p class="line-clamp leading-tight text-gray-600 dark:text-gray-200">
                {{ pkg.description }}
              </p>
            </router-link>
            <div class="mt-2 flex items-center text-gray-600 dark:text-gray-200">
    ...
    <ui-modal v-model="addState.show" :title="t('packages.add')" @close="clearNewPackage">
      <ui-input
...
const packages = computed(() => {
  const filtered = packageStore.packages.filter((item) => {
    const query = item.name.toLocaleLowerCase().includes(state.query.toLocaleLowerCase());
    return query;
  });
  return arraySorter({
    data: filtered,
...
const exportPackage = (pkg) => {
  ...
  const blobUrl = dataExporter(
    copyPkg,
    { type: 'json', name: `${pkg.name}.browsless-pkg` },
    true
  );
...
const deletePackage = ({ id, name }) => {
  dialog.confirm({
    ...
    body: `Are you sure you want to delete "${name}" from the library?`,
    okVariant: 'danger',
    okText: 'Delete',
    onConfirm: () => {
...
const addPackage = async () => {
  try {
    await packageStore.insert({
      name: addState.name.trim() || 'Unnamed block',
