      ...
      <tbody class="divide-y dark:divide-gray-800">
        <template v-if="running && running[0]?.state">
          <tr v-for="item in running" :key="item.id" class="border p-2">
            ...
            <td :title="t('log.duration')" class="log-time w-2/12 dark:text-gray-200">
        ...
        <tr v-for="log in logs" :key="log.id" class="hoverable">
          ...
          <td class="text-overflow w-4/12" style="min-width: 140px; max-width: 330px">
            <p
              v-if="modal"
              class="log-link text-overflow"
            ...
            <router-link v-else :to="`/logs/${log.id}`" class="log-link text-overflow">
              {{ log.name }}
            </router-link>
          </td>
          <td class="log-time w-3/12 dark:text-gray-200" style="min-width: 200px">
            <v-remixicon
...
const getTranslation = (key, defText = '') => {
  return te(key) ? t(key) : defText;
};
const stopWorkflow = (stateId) => {
  RendererWorkflowService.stopWorkflowExecution(stateId);
};
const toggleSelectedLog = (selected, id) => {
  if (selected) {
