<template>
  <div>
    ...
    />
    <ui-button class="mt-4 mb-2 w-full" variant="accent" @click="showModal = !showModal">
    ...
    >
      ...
      >
        <li v-for="(item, index) in dataList" :key="index" class="mb-4 rounded-lg border">
            ...
            <div v-if="hasFileAccess && item.isFile" class="flex items-end">
              <edit-autocomplete class="w-full">
                <ui-input
                  ...
                  class="w-full"
                  placeholder="File absolute path/File URL"
                />
              </edit-autocomplete>
              <template
                v-if="
...
const previewData = async (index, item) => {
  try {
