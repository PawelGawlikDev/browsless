<template>
  <div class="logs-list overflow-auto pb-4 pt-1">
    ...
    <ui-modal v-model="exportDataModal.show" content-class="max-w-2xl">
      ...
      <logs-data-viewer :log="exportDataModal.log" editor-class="logs-list-data" />
    </ui-modal>
  </div>
</template>
<script setup>
...
import { shallowReactive, ref, computed, watch, onMounted } from 'vue';
import { useI18n } from 'vue-i18n';
import { useDialog } from '@/composable/dialog';
import dbLogs from '@/db/logs';
import { useWorkflowStore } from '@/stores/workflow';
import { useLiveQuery } from '@/composable/liveQuery';
import LogsFilters from '@/components/dashboard/logs/LogsFilters.vue';
import LogsDataViewer from '@/components/dashboard/logs/LogsDataViewer.vue';
import SharedLogsTable from '@/components/dashboard/shared/SharedLogsTable.vue';
...
const allWorkflows = computed(() =>
  [...workflowStore.getWorkflows].sort((a, b) => (a.createdAt > b.createdAt ? -1 : 1))
);
...
const workflowStates = computed(() => {
  ...
  return states.filter((state) => state.workflowId === filtersBuilder.workflowId);
});
...
const toggleSelectedLog = (selected, logId) => {
  if (selected) {
...
watch(
  () => sortsBuilder,
  (value) => {
    setExtensionStorageValue('local', 'logs-sorts', value);
  },
  { deep: true }
);
onMounted(async () => {
  const savedSorts = (await getExtensionStorageValue('local', 'logs-sorts', {})) || {};
  sortsBuilder.order = savedSorts.order || 'desc';
  sortsBuilder.by = savedSorts.by || 'endedAt';
});
</script>
<style>
.logs-list-data {
