<template>
  <div class="mb-12">
    ...
    <div class="flex items-center space-x-4">
      ...
      >
        ...
        >
          <img :src="getThemeImage(item.id)" width="140" class="rounded-lg" />
        </div>
  ...
  <div class="flex items-center">
    <div id="languages">
      ...
      <ui-select :model-value="settings.locale" class="w-80" @change="updateLanguage">
        <option v-for="locale in supportLocales" :key="locale.id" :value="locale.id">
          {{ locale.name }}
        </option>
      </ui-select>
    </div>
  ...
  <div id="delete-logs" class="mt-12">
    ...
    <div class="flex items-center">
      <ui-select
        ...
        class="w-80"
        @change="updateSetting('deleteLogAfter', $event === 'never' ? 'never' : +$event)"
      >
...
const themeImages = import.meta.glob('../../../assets/images/theme-*.png', {
  eager: true,
  import: 'default',
});
const { t } = useI18n();
...
const getThemeImage = (id) => {
  return themeImages[`../../../assets/images/theme-${id}.png`];
};
